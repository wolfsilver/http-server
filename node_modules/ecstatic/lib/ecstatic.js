...
var ecstatic = module.exports = function (dir, options) {
  ...
  return function middleware (req, res, next) {
      ...
      fs.stat(file, function (err, stat) {
        if (err && (err.code === 'ENOENT' || err.code === 'ENOTDIR')) {
          if (req.statusCode == 404) {
            ...
            // 尝试public/404.html
            fs.exists(path.join(__dirname, '../../../public/404.html'), (exists) => {
              if (exists) {
                var resStream = fs.createReadStream(path.join(__dirname, '../../../public/404.html'));
                resStream.pipe(res);
          ...
          if (autoIndex) {
            req.url = urlJoin(encodeURIComponent(pathname), '/index.' + defaultExt);
    ...
    function serve(stat) {
      ...
      var resFile, resStream;

      // 请求中有callback参数，自动包裹json对象
      if (req.query && req.query.callback) {
        resFile = fs.readFileSync(file, charSet),
        resStream = new stream.Readable();
        // console.log(req.query.callback + '(' + resFile + ')');

        resStream.push(req.query.callback);
        resStream.push('(');
        resStream.push(resFile);
        resStream.push(')');
        resStream.push(null);
        ...
        if (shouldCompress(req)) {
          ...
          switch (acceptEncoding[0].trim()) {
            case 'gzip':
              res.setHeader('content-encoding', 'gzip');
              resStream = resStream.pipe(zlib.createGzip());
              break;
            case 'deflate':
              res.setHeader('content-encoding', 'deflate');
              resStream = resStream.pipe(zlib.createDeflate());
              break;
            default:
              break;
          }
      ...
      // include加载页面片
      resFile = fs.readFileSync(file, charSet);
      var getMatch = resFile.match(/<!--[\s]*#include[\s]*virtual="([^"]*)"[\s]*-->/);
      if (getMatch && getMatch.length > 1) {
        var includeFile = fs.readFileSync(path.join(file.substr(0, file.lastIndexOf('\\') + 1), getMatch[1]), charSet);
        resStream = new stream.Readable();

        resStream.push(resFile.replace(/<!--[\s]*#include[\s]*virtual="([^"]*)"[\s]*-->/, includeFile));
        resStream.push(null);
      ...
      resStream = fs.createReadStream(file);
